...
$LockPath = Join-Path $ProjectRoot "g2e\docs\P5A_D2S2_ONECLICK_LOCK.json"
...
$ScriptBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s2_isolated_volume_oneclick.ps1").Trim()
$PreflightBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s2_isolated_volume_preflight.py").Trim()
...
$LocalRoot = Join-Path $ProjectRoot "g2e\.local\P5A-D2S2"
...
$ReportJson = Join-Path $ReportDir "P5A_D2S2_ONECLICK_REPORT.json"
$ReportMd   = Join-Path $ReportDir "P5A_D2S2_ONECLICK_REPORT.md"
...
$VhdxPath = Join-Path $VolumeDir "P5A-D2S2-TASK.vhdx"
...
try {
    ...
    $diskpart = @(
        ...
        "format fs=ntfs quick label=G2ED2S2",
    ...
    $RootNames = @(Get-ChildItem -LiteralPath $VolumeRoot -Force | Select-Object -ExpandProperty Name | Sort-Object)
...
"@
    ...
    $PreflightScript = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s2_isolated_volume_preflight.py"
