...
EXECUTION_CONFIG_CANONICAL_SHA256 = "4af6909cecd412e8daf9c641222e129f1e35b7f94b00179339991a6f3ddb72ea"
TURN_TIMEOUT_S = 90.0
        ...
        for msg in list(notifications):
            if msg.get("method") == "turn/completed":
                ...
                if turn.get("id") in (None, turn_id):
                    return {"terminal": True, "timeout": False, "completion": msg, "notifications": notifications}
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            return {"terminal": False, "timeout": True, "completion": None, "notifications": notifications}
        try:
            msg = client.next_message(min(1.0, remaining))
        except TimeoutError:
            continue
        if "method" in msg and "id" in msg:
