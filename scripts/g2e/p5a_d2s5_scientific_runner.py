...
PREFLIGHT_MODULE_GIT_BLOB = "57f5b3e0a1ac13f7722d7bfe244a100bfeb08504"
...
def load_preflight_module(project_root: Path):
    path = project_root / "scripts" / "g2e" / "p5a_d2s5_observable_client.py"
    actual_blob = run_git(
        project_root,
        "rev-parse",
        "HEAD:scripts/g2e/p5a_d2s5_observable_client.py",
    )
    if actual_blob != PREFLIGHT_MODULE_GIT_BLOB:
        raise RuntimeError(f"PREFLIGHT_MODULE_BLOB_DRIFT:{actual_blob}")
    spec = importlib.util.spec_from_file_location("d2s5_observable_client_frozen", path)
    ...
    client.send(
        {
            ...
            "params": {
                "clientInfo": {
                    "name": "g2e_p5a_d2s5_science",
                    "title": "G2E P5A D2-S5 Science",
                    "version": "1.0.0",
                },
...
def main() -> int:
    ...
    if getattr(module, "OBSERVABILITY_GIT_BLOB", None) != OBSERVABILITY_GIT_BLOB:
        raise RuntimeError("SCIENCE_OBSERVABILITY_BINDING_DRIFT")
    ...
    marker_path = evidence_dir / "P5A_D2S5_TURN_START_SENT.marker"

    evidence = {
        "schema": "G2E-P5A-D2S5-SCIENTIFIC-RUNNER-v1",
        "study_id": STUDY_ID,
        ...
        "observable_client_git_blob": PREFLIGHT_MODULE_GIT_BLOB,
    ...
    try:
        ...
        out = evidence_dir / "P5A_D2S5_SCIENTIFIC_RUNNER_EVIDENCE.json"
        out.write_text(
