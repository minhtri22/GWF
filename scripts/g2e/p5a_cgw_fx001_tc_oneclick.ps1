...
if ([string]::IsNullOrWhiteSpace($HandoffDiagnostic)) {
    $HandoffDiagnostic = Join-Path $ProjectRoot "g2e\.local\P5A-CGW-FX001-TC-001-HANDOFF.json"
}
if ([string]::IsNullOrWhiteSpace($HandoffConfig)) {
    $HandoffConfig = Join-Path $ProjectRoot "g2e\.local\P5A-CGW-FX001-TC-001-HANDOFF-CONFIG.json"
}
...
if ($ElevatedChild) {
    trap {
        try {
            ...
            $markerCandidate = Join-Path $ProjectRoot "g2e\.local\P5A-CGW-FX001-TC-001\evidence\attempt_consumed.marker"
...
$DispatchLockPath = Join-Path $ProjectRoot "g2e\docs\P5A_CGW_FX001_TC_EXECUTION_LOCK.json"
$ExecutionConfigPath = Join-Path $ProjectRoot "g2e\config\P5A_CGW_FX001_TC_EXECUTION_CONFIG.json"
$AdmissionScript = Join-Path $ProjectRoot "scripts\g2e\p5a_cgw_fx001_tc_admission.py"
...
if (-not $PreflightOnly) {
    ...
    if ([string]$Lock.status -ne "DISPATCH_AUTHORIZED_EXECUTION_LOCK_TC1") { throw "TC_DISPATCH_LOCK_STATUS_INVALID" }
    ...
    if ((Get-GitBlob $ProjectRoot "scripts/g2e/p5a_cgw_fx001_tc_admission.py") -ne $Lock.components.admission_blob) { throw "TC_ADMISSION_BLOB_DRIFT" }
    ...
    if ((Get-GitBlob $ProjectRoot "scripts/g2e/p5a_cgw_fx001_tc_oneclick.ps1") -ne $Lock.components.oneclick_blob) { throw "TC_ONECLICK_BLOB_DRIFT" }
...
$LocalRoot = Join-Path $ProjectRoot "g2e\.local\P5A-CGW-FX001-TC-001"
...
$VhdxPath = Join-Path $VolumeDir "P5A-CGW-FX001-TC.vhdx"
...
$SummaryPath = Join-Path $ReportDir "P5A_CGW_FX001_TC_EXECUTION_REPORT.json"
