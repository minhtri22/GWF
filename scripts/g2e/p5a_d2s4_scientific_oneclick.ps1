...
$StudyId = "p5a-d2s4-queue-safe-scientific-successor"
$AttemptId = "p5a-d2s4-p5-fx-001-attempt-001"
...
function Invoke-InfrastructureSelfTest {
    $root = Join-Path ([IO.Path]::GetTempPath()) ("g2e-d2s4-science-" + [Guid]::NewGuid().ToString("N"))
    try {
...
$LockPath = Join-Path $ProjectRoot "g2e\docs\P5A_D2S4_SCIENTIFIC_EXECUTION_LOCK.json"
...
$OneclickBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s4_scientific_oneclick.ps1").Trim()
$RunnerBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s4_scientific_runner.py").Trim()
$VerifierBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s4_scientific_verify.py").Trim()
$AdmissionBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s4_scientific_admission.py").Trim()
$PreflightBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s4_queue_safe_client.py").Trim()
...
$LocalRoot = Join-Path $ProjectRoot "g2e\.local\P5A-D2S4-SCIENCE-001"
...
$ReportPath = Join-Path $ReportDir "P5A_D2S4_SCIENTIFIC_EXECUTION_REPORT.json"
$VhdxPath = Join-Path $VolumeDir "P5A-D2S4-SCIENCE-001-TASK.vhdx"
$DiskpartScript = Join-Path $VolumeDir "create_science_vhdx.diskpart.txt"

$Report = [ordered]@{
    schema = "G2E-P5A-D2S4-SCIENTIFIC-EXECUTION-REPORT-v1"
    status = "STARTED"
    study_id = $StudyId
...
try {
    try {
        ...
        $diskpart = @(
            ...
            "format fs=ntfs quick label=G2ED2S4S01",
...
"@
        ...
        $Runner = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s4_scientific_runner.py"
        $RunnerArgs = @(
            $Runner,
        ...
        $RunnerEvidence = Join-Path $EvidenceDir "P5A_D2S4_SCIENTIFIC_RUNNER_EVIDENCE.json"
        $Marker = Join-Path $EvidenceDir "P5A_D2S4_TURN_START_SENT.marker"
        ...
        if (Test-Path -LiteralPath $Marker -PathType Leaf) {
            ...
            $Verifier = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s4_scientific_verify.py"
            $VerificationPath = Join-Path $VerificationDir "P5A_D2S4_SCIENTIFIC_VERIFICATION.json"
            $VerifierArgs = @(
                $Verifier,
