...
PREDECESSOR_POSTCLOSURE_BUNDLE_SHA256 = "1775c3430692ec4e8037eb1776368d2544f5268d4dd71df77c63f3fca5c6eb66"
QUEUE_SAFE_CLIENT_GIT_BLOB = "6a7882dba93973aa77d8c4dd1823729d9d5d26f8"

STUDY_ID = "p5a-d2s4-queue-safe-scientific-successor"
ATTEMPT_ID = "p5a-d2s4-p5-fx-001-attempt-001"
PROFILE_ID = "g2e_p5a_d2s3"
REVISION_ID = "p5-fx-001-d2s4-v1"
...
def _provenance(discovery_digest: str) -> Provenance:
    return Provenance(
        created_by="g2e-p5a-d2s4-scientific-admission",
        created_at="2026-09-22T10:00:00Z",
        source_refs=(
            f"d2s4-structural-discovery:{discovery_digest}",
            ...
            f"queue-safe-client-git-blob:{QUEUE_SAFE_CLIENT_GIT_BLOB}",
        ),
        derivation=(
            "Prospective D2-S4 structural manifest, proof and P1.5 admission; "
...
def build_manifest(discovery: dict) -> AgentCapabilityManifest:
    ...
    capabilities = [
        AgentCapability(
            ...
            limitations=(
                "D2-S4 structural surface only; functional execution not demonstrated",
            ),
        )
    ...
    capabilities.extend(
        AgentCapability(
            ...
            available=False,
            qualification_refs=(),
            limitations=("D2-S4 bounded functional qualification required",),
        )
        for capability_id in FUNCTIONAL_UNQUALIFIED
    )

    return AgentCapabilityManifest.sealed(
        object_id="agent-capability-manifest-codex-d2s4-d1",
        revision_id="444a3f0008050605-d1-v1",
        provenance=provenance,
        agent_app="codex",
        profile_version="g2e-p5a-d2s4-d1-v1",
        ...
        limitations=(
            "availability applies only to D2-S4 structural App Server surface",
...
def build_proof_bundle(manifest: AgentCapabilityManifest, discovery_digest: str) -> dict:
    ...
    admission = _sealed(
        EvidenceAdmissionPolicy,
        "p5a-d2s4-evidence-admission",
        provenance,
        accepted_source_classes=("codex_p5_fx_001_d2s4",),
    ...
    proof = _sealed(
        ProofObligation,
        "p5a-d2s4-p5-fx-001-proof",
        provenance,
        lifecycle=ProofLifecycle.FROZEN,
        target_claim_id="p5a-d2s4-codex-functional-qualification",
        proposition=(
            "The exact release-coherent D2-S4 Codex harness completes P5-FX-001 "
            "within the frozen workspace, mutation, authority and evidence contract."
        ),
        assumptions=(
            f"exact D2-S4 harness sha256 {HARNESS_SHA256} remains unchanged",
...
def build_final_admission(discovery: dict) -> dict:
    ...
    authority_policy = _sealed(
        AuthorityPolicy,
        "p5a-d2s4-p5-fx-001-authority-policy",
        provenance,
        actions=(
            AuthorityAction(
    ...
    grant = _sealed(
        QualificationAuthorityGrant,
        "p5a-d2s4-p5-fx-001-qualification-grant",
        provenance,
        ...
        qualification_lineage_ref=(
            "g2e/docs/P5A_D2S4_SCIENTIFIC_EXECUTION_MACRO_SPEC.md"
        ),
    )

    binding = _sealed(
        AgentBinding,
        "p5a-d2s4-final-agent-binding",
        provenance,
        ...
        execution_constraints=(
            ...
            f"queue_safe_client_git_blob:{QUEUE_SAFE_CLIENT_GIT_BLOB}",
        ...
        resolution_ref="p5a-d2s4-scientific-macro-admission",
    )
    ...
    execution_config = {
        ...
        "queue_safe_client_git_blob": QUEUE_SAFE_CLIENT_GIT_BLOB,
    ...
    attempt = _sealed(
        ExecutionAttemptEnvelope,
        "p5a-d2s4-predispatch-attempt-envelope",
        provenance,
        attempt_id=ATTEMPT_ID,
        proof_ref=proof.exact_ref(),
    ...
    if capabilities.get("repository_read") is not False:
        raise ValueError("repository_read became available before D2-S4 result")
    if capabilities.get("repository_write") is not False:
        raise ValueError("repository_write became available before D2-S4 result")

    return {
        "schema": "G2E-P5A-D2S4-SCIENTIFIC-ADMISSION-v1",
        "study_id": STUDY_ID,
        "attempt_id": ATTEMPT_ID,
...
def verify_final_admission(pack: dict, discovery: dict) -> None:
    ...
    if pack != expected:
        raise ValueError("materialized D2-S4 admission is not deterministic")
...
def materialize(discovery_path: Path, output_dir: Path) -> dict:
    ...
    objects = {
        "P5A_D2S4_AGENT_CAPABILITY_MANIFEST.json": pack["manifest"],
        "P5A_D2S4_PROOF_OBLIGATION.json": pack["proof_objects"]["proof"],
        "P5A_D2S4_AUTHORITY_POLICY.json": pack["authority_policy"],
        "P5A_D2S4_QUALIFICATION_AUTHORITY_GRANT.json": (
            pack["qualification_authority_grant"]
        ),
        "P5A_D2S4_FINAL_AGENT_BINDING.json": pack["final_agent_binding"],
        "P5A_D2S4_PREDISPATCH_ATTEMPT.json": pack["predispatch_attempt"],
        "P5A_D2S4_SCIENTIFIC_ADMISSION.json": pack,
    }
