...
def verify(
    *,
    ...
    identity_checks = {
        "runner_schema": runner.get("schema") == "G2E-P5A-CGW-FX001-TC-RUNNER-v1",
    ...
    protocol_failure = bool(unexpected_server_requests)
    scope_violation = web_search_seen

    bridge_side_effects_seen = any(
        r.get("bridge_side_effects_performed") is True
        for r in route
    )
    non_codex_executor_seen = any(
        r.get("event") == "mcp_roundtrip"
        and isinstance(r.get("executor"), str)
        and r.get("executor") != "codex"
        for r in route
    )
    authority_violation = bridge_side_effects_seen or non_codex_executor_seen
    ...
    return {
        "schema": "G2E-P5A-CGW-FX001-TC-VERIFICATION-v1",
        "study_id": STUDY_ID,
