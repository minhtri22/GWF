...
$StudyId = "p5a-d2s5-observability-qualified-functional-successor"
$AttemptId = "p5a-d2s5-p5-fx-001-attempt-001"
...
function Invoke-InfrastructureSelfTest {
    $root = Join-Path ([IO.Path]::GetTempPath()) ("g2e-d2s5-science-" + [Guid]::NewGuid().ToString("N"))
    try {
...
if (-not (Test-IsAdministrator)) {
    Write-Host "Administrator elevation is required for the D2-S5 scientific isolated VHDX."
...
$LockPath = Join-Path $ProjectRoot "g2e\docs\P5A_D2S5_SCIENTIFIC_EXECUTION_LOCK.json"
...
$OneclickBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s5_scientific_oneclick.ps1").Trim()
$RunnerBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s5_scientific_runner.py").Trim()
$VerifierBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s5_scientific_verify.py").Trim()
$AdmissionBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s5_scientific_admission.py").Trim()
$PreflightBlob = (git rev-parse "HEAD:scripts/g2e/p5a_d2s5_observable_client.py").Trim()
$ObservabilityBlob = (git rev-parse "HEAD:scripts/g2e/p5a_codex_observability.py").Trim()
...
if ($ObservabilityBlob -ne $Lock.observability_blob) { throw "OBSERVABILITY_BLOB_DRIFT" }
...
$LocalRoot = Join-Path $ProjectRoot "g2e\.local\P5A-D2S5-SCIENCE-001"
...
$ReportPath = Join-Path $ReportDir "P5A_D2S5_SCIENTIFIC_EXECUTION_REPORT.json"
$VhdxPath = Join-Path $VolumeDir "P5A-D2S5-SCIENCE-001-TASK.vhdx"
$DiskpartScript = Join-Path $VolumeDir "create_science_vhdx.diskpart.txt"

$Report = [ordered]@{
    schema = "G2E-P5A-D2S5-SCIENTIFIC-EXECUTION-REPORT-v1"
    status = "STARTED"
    study_id = $StudyId
    ...
    observability_blob = $Lock.observability_blob
...
try {
    try {
        ...
        $diskpart = @(
            ...
            "format fs=ntfs quick label=G2ED2S5S01",
...
description = "G2E P5A D2-S5 scientific isolated-volume authority"
...
"@
        ...
        $Runner = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s5_scientific_runner.py"
        $RunnerArgs = @(
            $Runner,
        ...
        $RunnerEvidence = Join-Path $EvidenceDir "P5A_D2S5_SCIENTIFIC_RUNNER_EVIDENCE.json"
        $Marker = Join-Path $EvidenceDir "P5A_D2S5_TURN_START_SENT.marker"
        ...
        if (Test-Path -LiteralPath $Marker -PathType Leaf) {
            ...
            $Verifier = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s5_scientific_verify.py"
            $VerificationPath = Join-Path $VerificationDir "P5A_D2S5_SCIENTIFIC_VERIFICATION.json"
            $VerifierArgs = @(
                $Verifier,
...
Write-Host ""
Write-Host "=== D2-S5 SCIENTIFIC EXECUTION COMPLETE ==="
