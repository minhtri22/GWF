...
if ($ConfigSerializationSelfTest) {
    $sample = "alpha`r`nbeta"
    $normalized = Convert-ToLfText $sample
    if ($normalized -ne "alpha`nbeta") {
        throw "CONFIG_SERIALIZATION_SELFTEST_VALUE_DRIFT"
    }
    if ($normalized.Contains("`r")) {
        throw "CONFIG_SERIALIZATION_SELFTEST_CR_REMAINS"
    }
...
$LockPath = Join-Path $ProjectRoot "g2e\docs\P5A_D2S2_A2_ONECLICK_LOCK.json"
...
$ScriptBlob = (Invoke-Git -Root $ProjectRoot -GitArgs @("rev-parse","HEAD:scripts/g2e/p5a_d2s2_a2_config_serialization_oneclick.ps1")).Trim()
$PreflightBlob = (Invoke-Git -Root $ProjectRoot -GitArgs @("rev-parse","HEAD:scripts/g2e/p5a_d2s2_a2_config_serialization_preflight.py")).Trim()
...
$LocalRoot = Join-Path $ProjectRoot "g2e\.local\P5A-D2S2-A2"
if (Test-Path $LocalRoot) {
    throw "D2S2_LOCAL_ROOT_ALREADY_EXISTS:$LocalRoot"
}

$VolumeDir = Join-Path $LocalRoot "volume"
$CodexHome = Join-Path $LocalRoot "codex-home\preturn-a2"
$EvidenceDir = Join-Path $LocalRoot "evidence\preturn-a2"
...
$ReportJson = Join-Path $ReportDir "P5A_D2S2_A2_ONECLICK_REPORT.json"
$ReportMd   = Join-Path $ReportDir "P5A_D2S2_A2_ONECLICK_REPORT.md"
...
$VhdxPath = Join-Path $VolumeDir "P5A-D2S2-A2-TASK.vhdx"
...
try {
    ...
    $diskpart = @(
        ...
        "format fs=ntfs quick label=G2ED2S2A2",
...
enabled = false
"@
    $configLf = Convert-ToLfText $config
    [IO.File]::WriteAllText($ConfigPath, $configLf + "`n", [Text.UTF8Encoding]::new($false))
    ...
    $PreflightScript = Join-Path $ProjectRoot "scripts\g2e\p5a_d2s2_a2_config_serialization_preflight.py"
    ...
    $EvidenceFile = Join-Path $EvidenceDir "P5A_D2S2_A2_LOCAL_PRETURN_PREFLIGHT.json"
