...
def _provenance(discovery_digest: str) -> Provenance:
    return Provenance(
        created_by="g2e-p5a-d2s3-scientific-admission",
        created_at="2026-09-22T10:00:00Z",
        source_refs=(
            f"d2s3-structural-discovery:{discovery_digest}",
            f"release-tag:{RELEASE_TAG}",
            f"source-commit:{SOURCE_COMMIT}",
            f"r2-report-sha256:{R2_REPORT_SHA256}",
            f"r2-evidence-sha256:{R2_EVIDENCE_SHA256}",
        ),
        derivation=(
            "Prospective D2-S3 structural manifest, proof and P1.5 admission; "
...
def build_manifest(discovery: dict) -> AgentCapabilityManifest:
    ...
    capabilities = [
        AgentCapability(
            ...
            limitations=(
                "D2-S3 structural surface only; functional execution not demonstrated",
            ),
        )
    ...
    capabilities.extend(
        AgentCapability(
            ...
            available=False,
            qualification_refs=(),
            limitations=("D2-S3 bounded functional qualification required",),
        )
        for capability_id in FUNCTIONAL_UNQUALIFIED
    )

    return AgentCapabilityManifest.sealed(
        object_id="agent-capability-manifest-codex-d2s3-d1",
        revision_id="444a3f0008050605-d1-v1",
        provenance=provenance,
        agent_app="codex",
        profile_version="g2e-p5a-d2s3-d1-v1",
        ...
        limitations=(
            "availability applies only to D2-S3 structural App Server surface",
...
def build_proof_bundle(manifest: AgentCapabilityManifest, discovery_digest: str) -> dict:
    ...
    proof = _sealed(
        ...
        proposition=(
            "The exact release-coherent D2-S3 Codex harness completes P5-FX-001 "
            "within the frozen workspace, mutation, authority and evidence contract."
        ),
        assumptions=(
            f"exact D2-S3 harness sha256 {HARNESS_SHA256} remains unchanged",
...
def build_final_admission(discovery: dict) -> dict:
    ...
    binding = _sealed(
        ...
        execution_constraints=(
            ...
            f"r2_evidence_sha256:{R2_EVIDENCE_SHA256}",
        ...
        resolution_ref="p5a-d2s3-scientific-macro-admission",
    )
    ...
    if capabilities.get("repository_read") is not False:
        raise ValueError("repository_read became available before D2-S3 result")
    if capabilities.get("repository_write") is not False:
        raise ValueError("repository_write became available before D2-S3 result")

    return {
        "schema": "G2E-P5A-D2S3-SCIENTIFIC-ADMISSION-v1",
        "study_id": STUDY_ID,
        "attempt_id": ATTEMPT_ID,
        ...
        "authorization": {
            "mode": (
                "CONDITIONAL_ON_EXACT_R2_EVIDENCE_AND_FRESH_RUNTIME_PREFLIGHT"
            ),
            "r2_report_sha256": R2_REPORT_SHA256,
            "r2_evidence_sha256": R2_EVIDENCE_SHA256,
...
def verify_final_admission(pack: dict, discovery: dict) -> None:
    ...
    if pack != expected:
        raise ValueError("materialized D2-S3 admission is not deterministic")
...
def materialize(discovery_path: Path, output_dir: Path) -> dict:
    ...
    objects = {
        "P5A_D2S3_AGENT_CAPABILITY_MANIFEST.json": pack["manifest"],
        "P5A_D2S3_PROOF_OBLIGATION.json": pack["proof_objects"]["proof"],
        "P5A_D2S3_AUTHORITY_POLICY.json": pack["authority_policy"],
        "P5A_D2S3_QUALIFICATION_AUTHORITY_GRANT.json": (
            pack["qualification_authority_grant"]
        ),
        "P5A_D2S3_FINAL_AGENT_BINDING.json": pack["final_agent_binding"],
        "P5A_D2S3_PREDISPATCH_ATTEMPT.json": pack["predispatch_attempt"],
        "P5A_D2S3_SCIENTIFIC_ADMISSION.json": pack,
    }
